...
![image-20211221160553934](C:\Users\thangno\AppData\Roaming\Typora\typora-user-images\image-20211221160553934.png)
...
function sayHello() {
  let friend = "Sohn";
  //함수안에도 변수를 선언하고 값을 할당하는것이 가능하다.

  console.log("Hello!" + friend);
  //어차피 문자열이기 때문에 구분자 , 을 사용해도 무방하다.
}

sayHello();
```

#### 2.return(output)이 있는 함수
...
데이터를 받으면 대체되는 자리표시자 일 뿐임.

```js
//함수의 알규먼트는 기존에 설정된 변수명을 적어도 된다.
//ex)
const age = 96;
...
const korAge = calculatekorAge(age); //위에 선언된 변수 age가 들어갔다.
console.log(korAge);
```
...
let minusResult = calculator.minus(addResult, 2);
console.log(minusResult); //addResult -1 =1
```
